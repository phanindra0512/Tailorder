...
function CompletedPayments({ navigation }) {
	...
	useEffect(() => {
		const todayOrders = async () => {
			...
			const dateByData = orders.filter((d) => {
				return d.orderDetails.orderStatus == 'DELIVERIED';
			});
			console.log(dateByData.length);
			setIsOrders(dateByData);
